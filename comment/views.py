...
class MenuItemCommentView(views.APIView):
    ...
    def get(self, request, item_id):
        try:
            comments = Comment.objects.filter(item_id=item_id, is_cafe=False).order_by(
                "-date"
            )
            if not comments:
                return Response(
                    {"message": "کامنتی ثبت نشده است"}, status=status.HTTP_404_NOT_FOUND
                )
            serializer = CommentSerializer(comments, many=True)
            return Response({"data": serializer.data}, status=status.HTTP_200_OK)
        except:
            return Response(
                {"message": "مشکلی ایجاد شده"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def get(self, request, id):
        try:
            ...
            is_bartender = Bartender.objects.filter(
                cafe=cafe, user=request.user, is_active=True
            ).exists()
            ...
            return Response(
                {"data": "به کامنت های دیگران دسترسی مجاز نیست"},
    ...
    def delete(self, request, id):
        try:
            ...
            if cafe.owner == request.user or is_bartender:
                comment.delete()
                return Response(
                    {"data": "کامنت با موفقیت حذف شد"}, status=status.HTTP_200_OK
                )
    ...
    def get_queryset(self):
        ...
        if cafe.exists():
            return self.queryset.filter(
                cafe_id=cafe.first().id, is_cafe=False
            ).order_by("-date")

        bartender = Bartender.objects.filter(user=user, is_active=True)
        if bartender.exists():
            return self.queryset.filter(
                cafe_id=bartender.first().cafe.id, is_cafe=False
            ).order_by("-date")
