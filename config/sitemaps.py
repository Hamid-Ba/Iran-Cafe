...
def build_sitemap():
    sitemap = {"static": StaticSitemap, "cafes": CafesSitemap, "cafe": CafeSitemap}
    return sitemap
...
class CafesSitemap(BaseSitemap):
    ...
    def items(self):
        ...
        for province in province_models.Province.objects.all():
            data.append(urlencode({"province": province.slug, "id": province.id}))
        return data
